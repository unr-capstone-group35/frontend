  ...
  actions: {
    ...
    async submitAnswer(answer: any): Promise<boolean> {
      ...
      try {
        ...
        const response = await pointsStore.submitExerciseAttempt(courseId, lessonId, exerciseId, answer);
        ...
        if (response.isCorrect) {
          ...
          if (currentLesson) {
            const isLastExercise =
              currentLesson.exercises.findIndex((ex) => ex.id === exerciseId) === currentLesson.exercises.length - 1;

            // Update lesson status based on whether this is the last exercise
            await lessonStore.updateLessonStatus(courseId, lessonId, isLastExercise ? "completed" : "in_progress");
