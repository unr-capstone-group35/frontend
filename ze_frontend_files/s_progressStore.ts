...
export const useProgressStore = defineStore('progress', {
  ...
  getters: {
    ...
    getCourseProgress: (state) => (courseId: string): CourseProgress | null => {
      return state.courseProgress[courseId] || null
    },

    // Get progress for a specific lesson
    getLessonProgress: (state) => (courseId: string, lessonId: string): LessonProgress | null => {
      const key = `${courseId}-${lessonId}`
      return state.lessonProgress[key] || null
    },

    // Get all attempts for a specific exercise
    getExerciseAttempts: (state) => (courseId: string, lessonId: string, exerciseId: string): ExerciseAttempt[] => {
    ...
    getLatestExerciseAttempt: (state) => (courseId: string, lessonId: string, exerciseId: string): ExerciseAttempt | null => {
    ...
    isLessonCompleted: (state) => (courseId: string, lessonId: string): boolean => {
      const key = `${courseId}-${lessonId}`
